...
du même checkpoint de départ dans les deux cas) : la \emph{durée} n'explique pas
l'écart (8 contre 30 époques donnent un résultat statistiquement équivalent), mais la
\emph{granularité} compte réellement -- personnaliser uniquement la tête, à partir du
même extracteur, surpasse nettement le fine-tuning du réseau entier, probablement en
limitant le surapprentissage sur le peu de données locales disponibles par client.
...
\subsection{Durée ou granularité ? La durée est écartée, la granularité compte
réellement}
\label{subsec:duration_vs_granularity}

Pour trancher lequel des deux facteurs ci-dessus domine, deux tests successifs sont
menés hors ligne sur FedProx+DP ($\sigma=0{,}8$, checkpoint final, 5 graines), sur les
données d'entraînement locales de chacun des 4 clients informatifs.

\textbf{Premier test, la durée seule} : fine-tuning du réseau entier (même granularité
que l'ablation post-hoc originale, Tableau~\ref{tab:ablation_b}), 8 contre 30 époques.
Résultat net : la balanced-accuracy ne bouge pas de façon significative entre les deux
durées -- \textbf{la durée n'est pas le facteur explicatif}. Ce test isole correctement
la durée (même checkpoint de départ, seule la durée varie) et cette conclusion est
solide.

\textbf{Second test, la granularité isolée correctement} -- un premier test comparant
naïvement le fine-tuning complet à FedPer (Tier 1) confondait la granularité avec le
fait que FedPer part de \emph{son propre} extracteur, entraîné selon un régime
d'agrégation entièrement différent (30 rounds sans terme proximal, jamais influencé par
un `fc3` agrégé) : une comparaison entre deux modèles de base différents, pas une
comparaison à granularité isolée. Corrigé en repartant du \textbf{même} checkpoint
FedProx+DP dans toutes les conditions, en ne faisant varier que ce qui est dégelé
...
\begin{tabular}{lcc}
  ...
  \midrule
  Réseau entier dégelé      & $47{,}86\,\%$ & $49{,}48\,\%$ \\
  \textbf{Tête seule (\texttt{fc3}) dégelée} & $\mathbf{56{,}48\,\%}$ & $\mathbf{56{,}73\,\%}$ \\
  \bottomrule
\end{tabular}
\end{center}
{\footnotesize Balanced-accuracy pooled sur les 4 clients informatifs. Référence :
FedProx+DP non adapté $42{,}73\,\%$ ; FedPer (son propre extracteur, 30 rounds continus)
$48{,}44\,\%$.}

\textbf{À partir du même point de départ, la personnalisation tête-seule surpasse
nettement le fine-tuning du réseau entier} -- et surpasse même la tête de FedPer
entraînée en continu sur son propre extracteur. \textbf{La granularité compte donc
réellement}, dans le sens inverse de ce qu'on pourrait attendre naïvement (moins de
paramètres ajustés fait mieux, pas moins bien) : avec très peu de données locales par
client, ajuster seulement les 34 paramètres de la tête limite le surapprentissage que
risque le réseau entier (1\,362 paramètres) sur un jeu de données aussi restreint --
une explication plausible, cohérente avec les pratiques usuelles de fine-tuning sur
domaine cible réduit, mais non vérifiée plus avant ici. Un point de rigueur relevé en
construisant ce test : le fine-tuning n'étant pas explicitement re-seedé à chaque appel
(l'ordre de mélange des mini-lots varie), les valeurs exactes de ce tableau comportent
une part de bruit d'exécution supplémentaire, non quantifiée -- sans remettre en cause
l'écart entre les deux granularités, large devant ce bruit.

Ce résultat révise la lecture de la Section~\ref{subsec:fedper} : l'avantage de FedPer
ne vient pas seulement du fait qu'une personnalisation ait lieu, mais bien en partie de
sa \emph{granularité} (tête seule) -- un choix de conception qui s'avère pertinent, pas
un simple détail d'implémentation. Il reste vrai, en revanche, que sa durée
d'exposition (30 rounds continus) n'est pas ce qui explique sa performance.
...
\begin{itemize}
  ...
  du même checkpoint.} L'écart initialement observé entre FedPer et FedProx fine-tuné
  (Section~\ref{subsec:fedper}) aurait pu refléter soit la \emph{durée} de
  personnalisation, soit la \emph{granularité}. Isolés l'un après l'autre
  (Section~\ref{subsec:duration_vs_granularity}), en partant à chaque fois du même
  checkpoint FedProx+DP pour ne faire varier qu'un seul facteur à la fois : la durée
  n'explique pas l'écart (8 contre 30 époques, résultat équivalent), mais la
  granularité si -- personnaliser uniquement la tête surpasse nettement le fine-tuning
  du réseau entier, à extracteur de départ identique. Un premier test comparant
  directement FedPer (son propre extracteur) au fine-tuning complet de FedProx avait
  conclu par erreur que ni l'un ni l'autre ne comptait ; cette comparaison confondait
  la granularité avec le fait que FedPer part d'un extracteur entraîné selon un régime
  différent -- corrigé en isolant chaque facteur à partir d'un point de départ commun.
  Le fine-tuning effectué pour ce test n'étant pas explicitement re-seedé, les valeurs
  exactes comportent une part de bruit d'exécution non quantifiée, sans remettre en
  cause l'écart mesuré entre granularités.
