...
centralisée non contrainte (même modèle, mêmes 5 graines, sans FL ni DP). Cette
baseline sert de plafond de référence théorique, non déployable en pratique sur ce
type de données de santé au sens du RGPD, mais utile pour quantifier honnêtement le
...
mais que \textbf{l'hétérogénéité extrême entre infirmières limite la généralisation de
...
réduire l'effet. Cette lecture fait de ce travail une étude de cas sur les limites
...
\textbf{Nuance empirique} : en pratique, les modèles entraînés sur ce jeu de données
apprennent un biais fort vers la classe majoritaire globale (stress, $86\,\%$ du
pooled). Ce biais produit une accuracy quasi-parfaite sur les 10 des 11 clients
mono-classe dont l'unique classe présente est cette classe majoritaire, mais joue dans
le sens opposé sur le client 3, seul mono-classe sur la classe minoritaire
(non-stress) : son accuracy y est proche de $0\,\%$ avec FL+DP (et $59{,}8\,\%$ avec le
centralisé), pas $100\,\%$. Conséquence directe pour la suite de ce document :
\textbf{toute métrique regroupant les 15 clients est fortement déformée par ces 11
clients triviaux} (tirée vers le haut par la plupart, vers le bas par le client 3), et
ne reflète qu'imparfaitement la performance clinique réelle. Chaque tableau de résultats de
...
\emph{après} découpage individuel avec le gap anti-fuite. C'est exactement la même
méthodologie que celle déjà utilisée pour bâtir le test-set du centralisé, ce qui
garantit une comparaison à armes égales. La précision brute étant peu informative sur un
...
clients ont un test-set mono-classe (Section~\ref{subsec:dataset}), où le biais du
modèle vers la classe majoritaire globale produit une accuracy quasi-parfaite pour la
plupart d'entre eux (à l'exception du client 3, seul mono-classe sur la classe
minoritaire ; voir la nuance détaillée Section~\ref{subsec:dataset}). Nous rapportons donc systématiquement,
...
\begin{table}[h!]
  ...
  \begin{tabular}{lcc}
    ...
    Précision INT8 (interne, pondérée par client) & $81{,}57 \pm 0{,}31\,\%$ & $81{,}43 \pm 1{,}13\,\%$ \\
...
\begin{figure}[h!]
  ...
  \caption{Balanced-accuracy FL vs centralisé, métrique pooled (15 clients) contre
  métrique restreinte aux 4 clients informatifs. L'avantage apparent du centralisé sur
  la métrique pooled se réduit fortement une fois restreint aux clients réellement
  informatifs, et les deux conditions avoisinent le niveau du hasard.}
...
clients informatifs (Figure~\ref{fig:main_comparison}), \textbf{le centralisé s'effondre lui aussi} ($45{,}73\,\%$, à peine
...
\begin{figure}[h!]
  ...
  \caption{Compromis confidentialité/utilité : le budget $\varepsilon$ décroît
  fortement avec $\sigma$ (échelle de gauche) tandis que la balanced-accuracy pooled
  (échelle de droite) reste quasiment plate sur toute la plage testée.}
...
sur toute la plage (Figure~\ref{fig:privacy_utility}) : le compromis confidentialité/utilité est empiriquement plat,
...
et 3 graines, contre 5 graines partout ailleurs dans ce rapport. C'est un
\textbf{cas illustratif}, pas un résultat généralisable à l'ensemble des 15
infirmières : rien ne garantit qu'un
...
8 époques d'adaptation) sur le client 5, l'un des 4 clients informatifs, où se
concentre une part substantielle ($12{,}7\,\%$) des erreurs confiantes du modèle
FL+DP global (détail complet par client Section~\ref{subsec:why_cent_wins}).
...
\begin{table}[h!]
  ...
  fenêtres non-stress correctement classées à chaque fois), pas une plage arrondie.
...
chacun, contre 5 graines partout ailleurs dans ce rapport. Ce sont des
\textbf{cas illustratifs}, pas un résultat généralisable aux 15 infirmières.
...
(5,3~\%). Ces deux échecs ont des mécanismes différents (un classifieur non entraîné,
pas un extracteur défaillant) : il ne faut pas les confondre avec un signal de mauvaise
qualité de l'extracteur.
...
le modèle final après 30 rounds d'apprentissage. La Figure~\ref{fig:dynamic_range}
...
\begin{table}[h!]
  ...
  \begin{tabular}{cccc}
    ...
    0,8 & Modéré-faible & $0{,}18 \pm 0{,}00$~Mo & $20{,}48 \pm 1{,}50$~Mo \\
...
avec le centralisé. C'est une nuance par rapport à la métrique pooled, où l'essentiel de
l'écart était attribué à la décentralisation plutôt qu'à la confidentialité, cohérente
...
\paragraph{Concentration des erreurs, revisitée.} Une inspection par client des
fenêtres où le modèle FL+DP est confiant à tort (probabilité prédite $>0{,}9$ pour la
classe prédite), agrégée sur les 25 checkpoints de la grille, montre que le client 3
concentre à lui seul $31{,}1\,\%$ de l'ensemble de ces erreurs, premier contributeur
sur les 25 checkpoints sans exception, suivi des 4 clients informatifs (client 12 :
$29{,}3\,\%$, client 8 : $19{,}4\,\%$, client 5 : $12{,}7\,\%$, client 4 : $6{,}3\,\%$) ;
à eux cinq, ces clients totalisent $98{,}9\,\%$ des erreurs confiantes, les 10 autres
clients mono-classe n'en représentant collectivement qu'environ $1\,\%$. Le mécanisme
diffère selon le cas : sur les 4 clients informatifs, il s'agit d'un signal réellement
difficile à apprendre (paragraphes précédents) ; sur le client 3, c'est le même biais
de classe majoritaire déjà noté Section~\ref{subsec:dataset} qui joue cette fois en sa
défaveur (son unique classe présente en test est la classe minoritaire globale), un
phénomène plus mécanique que les 4 clients informatifs mais qui n'en confirme pas moins
le constat central : les erreurs du modèle FL+DP se concentrent presque exclusivement
sur les clients où il y a réellement quelque chose de non trivial à réussir ou à rater
(qu'il s'agisse d'un signal difficile ou d'un biais de classe qui s'inverse), pas sur
les 10 clients mono-classe alignés avec le biais majoritaire. Ce constat s'inscrit dans
un problème plus large touchant aussi le centralisé, pas une pathologie propre au FL.
...
puisque chaque wearable exécuterait de toute façon son propre modèle.
...
Sur la métrique pooled, l'écart est très marqué (Welch $t=30{,}69$,
$p=3{,}1\times10^{-21}$ ; Mann-Whitney $U=625/625$, séparation complète), mais ce
...
$p<0{,}02$).
L'accuracy brute progresse elle aussi, mais de façon marginale et à peine significative
($+0{,}95$ point, $p=4{,}0\times10^{-2}$) : FedPer n'échange donc pas la précision brute
contre du rappel équilibré, mais ce gain conjoint sur l'accuracy est ténu, à la
différence du gain net et hautement significatif observé sur la balanced-accuracy et
l'AUC.
...
terme proximal, jamais influencé par un \texttt{fc3} agrégé). C'est corrigé ci-dessous en
...
\begin{figure}[h!]
  ...
  \caption{Balanced-accuracy sur les 4 clients informatifs, réseau entier dégelé
  contre tête seule dégelée, à 8 et 30 époques, à partir du même checkpoint de départ.
  La ligne pointillée marque le niveau FedProx+DP non adapté.}
...
D'après la Figure~\ref{fig:duration_granularity}, un test $t$ apparié (5 graines, chaque graine comparée à elle-même entre conditions)
...
$p=1{,}2\times10^{-2}$). \textbf{C'est une révision par rapport à un premier test non
...
les poids en quelques rounds. Ceci est corrigé en bornant la norme de la correction à
la même échelle que le clipping DP avant de l'ajouter au gradient, validé sur un
balayage étendu (12 rounds, pas seulement les 3 rounds du test rapide initial) avant de
rejouer la grille complète : 25/25 runs, aucun NaN, aucun échec.
...
signalée explicitement en synthèse (Section~\ref{subsec:synthesis_limits}) plutôt que
laissée implicite. Une seconde métrique, restreinte aux 4 clients informatifs, est donc
...
\begin{figure}[h!]
  ...
  \caption{Balanced-accuracy des trois architectures, métrique pooled contre métrique
  restreinte aux 4 clients informatifs. L'écart entre les deux barres de chaque paire
  mesure directement le gonflement dû aux 11 clients mono-classe.}
  \label{fig:perso_comparison}
\end{figure}

Sur les clients informatifs, la mesure la plus honnête cliniquement, la hiérarchie
FedProx+DP $<$ FedPer $<$ FedRep+SCAFFOLD est statistiquement nette (Figure~\ref{fig:perso_comparison} ; Welch et
...
$0{,}97\,\%$ pour FedProx+DP). Ceci a été vérifié non-artefactuel : les checkpoints de
graines différentes ont été comparés directement et diffèrent réellement, pas de
recyclage de fichier.
...
Ce constat est propre à l'architecture MLP Tiny testée ici et ne doit pas être
généralisé au-delà sans validation (voir Limites, Section~\ref{subsec:synthesis}).
...
Comme le montre la Figure~\ref{fig:dynamic_range}, cette régularisation compresse
fortement la plage dynamique des poids par rapport au centralisé. À $\sigma = 0{,}8$,
la plage dynamique globale du modèle FL + DP est de $3{,}37 \pm 0{,}17$ (contre
$9{,}45 \pm 1{,}03$ pour le centralisé, soit un facteur $\approx 2{,}8$), et tombe à
$0{,}96 \pm 0{,}07$ sur la première couche (contre $7{,}11 \pm 0{,}57$). Les poids du
...
ont un test-set mono-classe (Section~\ref{subsec:dataset}), qui déforme fortement
...
d'ampleur modeste comparé au gonflement apparent sur la métrique pooled, +17,2 points),
au prix d'une contrainte de déploiement propre à cette architecture (pas de modèle
global unique, chaque client garde sa propre tête de classification) ; (5bis)~un test
complémentaire isolant la granularité (Section~\ref{subsec:duration_vs_granularity})
montre qu'un simple fine-tuning post-hoc de la seule tête de classification, sur le
checkpoint FedProx+DP standard \emph{déjà entraîné} et sans architecture dédiée,
atteint $55{,}88 \pm 2{,}35\,\%$, \textbf{davantage que FedPer lui-même}, sans le
chemin d'entraînement fédéré spécifique ni la contrainte de déploiement associée. Ceci
suggère que l'essentiel du bénéfice ne vient pas de la construction FedPer en tant que
telle, mais de la possibilité de personnaliser légèrement la tête après coup, une
piste probablement plus simple à déployer en pratique ; et (6)~pousse cette logique
plus loin hors contrainte TinyML (Section~\ref{subsec:fedrep_scaffold}) en
...
l'hétérogénéité elle-même, pas seulement le choix d'architecture FL. Le point (5bis)
mérite d'être retenu comme le résultat le plus directement actionnable pour un
déploiement réel : un post-traitement léger (geler l'extracteur, fine-tuner la tête sur
les données locales après réception du modèle global standard) suffit à obtenir
l'essentiel du gain de personnalisation, avec une charge d'ingénierie nettement moindre
que celle de FedPer ou de FedRep+SCAFFOLD.
...
\begin{itemize}
  ...
  corrigée dans ce document, pas seulement documentée.} Découvert lors de l'évaluation de
  ...
  $\to$ 3 points) et FedPer-vs-FedProx (17,2 points $\to$ 5,7 points). C'est un changement
