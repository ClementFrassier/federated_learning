...
{\bf Results:}
Sur un test-set groupé (toutes infirmières confondues, seule méthode d'évaluation
comparable entre les deux conditions), le modèle fédéré atteint une précision de
$82{,}0 \pm 0{,}5\,\%$ en moyenne sur les 5 valeurs de $\sigma$, contre
$81{,}95 \pm 0{,}89\,\%$ pour le centralisé -- des précisions brutes statistiquement
indiscernables. Sur des métriques équilibrées par classe, en revanche, le centralisé
conserve un avantage net et stable ($57{,}8 \pm 1{,}2\,\%$ de balanced-accuracy contre
$\approx 47{,}9\,\%$ pour le FL, quel que soit $\sigma$). Une ablation FedProx sans
DP-SGD isole l'origine de cet écart : le modèle fédéré sans confidentialité obtient une
balanced-accuracy quasi identique ($48{,}7\,\%$) à celle obtenue avec DP-SGD, ce qui
montre que le coût mesuré vient essentiellement de la \textbf{décentralisation} (agréger
des mises à jour locales à travers 15 clients très hétérogènes) et non du bruit de
confidentialité, qui s'ajoute ensuite quasiment gratuitement. Ce coût reste stable sur
toute la plage de budget testée -- le compromis confidentialité/utilité est
empiriquement plat, de $\varepsilon = 5{,}29 \pm 0{,}30$ ($\sigma = 0{,}6$) jusqu'à
$\varepsilon = 0{,}54 \pm 0{,}04$ ($\sigma = 1{,}8$).

{\bf Conclusions:}
Le FL différentiellement privé n'égale pas le centralisé en pouvoir discriminant
équilibré sur ce jeu de données fortement Non-IID, mais ce n'est pas la question
pertinente : l'agrégation centralisée de signaux physiologiques d'infirmières est
elle-même juridiquement problématique au regard du RGPD, ce qui fait du FL la seule
approche viable, et non un concurrent à départager sur la précision. Nos résultats
quantifient honnêtement le coût de cette approche (environ 10 points de
balanced-accuracy, attribuable surtout à la fédération plutôt qu'à la confidentialité
elle-même) et montrent qu'il reste stable quel que soit le niveau de confidentialité
visé, tout en conservant un modèle déployable sur microcontrôleur (5,32~Ko).
\end{abstract}
...
86,0~\%), nous rapportons également la balanced-accuracy et le F1 macro.
...
\begin{table}[h!]
  ...
  \begin{tabular}{lcc}
    ...
    Balanced-accuracy            & $47{,}9 \pm 0{,}2\,\%$            & $\mathbf{57{,}84 \pm 1{,}16\,\%}$ \\
    F1 macro                     & $45{,}4 \pm 0{,}2\,\%$            & $\mathbf{58{,}56 \pm 1{,}26\,\%}$ \\
...
\begin{table}[h!]
  ...
  \begin{tabular}{lccc}
    ...
    Balanced-accuracy & $\mathbf{57{,}84 \pm 1{,}16\,\%}$ & $48{,}68 \pm 0{,}73\,\%$ & $47{,}83 \pm 0{,}35\,\%$ \\
    F1 macro          & $\mathbf{58{,}56 \pm 1{,}26\,\%}$ & $46{,}10 \pm 0{,}75\,\%$ & $45{,}33 \pm 0{,}28\,\%$ \\
    \bottomrule
  \end{tabular}
\end{table}

Le résultat est net : FL sans DP (48,68~\%) et FL + DP (47,83~\%) sont à moins d'un
point l'un de l'autre en balanced-accuracy, un écart plus petit que leurs écarts-types
respectifs. **Le bruit DP-SGD n'explique donc quasiment pas le coût mesuré.** L'écart
d'environ 10 points avec le centralisé est déjà presque entièrement présent sans aucun
DP-SGD -- c'est la fédération elle-même (l'agrégation FedAvg/FedProx à travers 15
clients à la composition de classe extrêmement hétérogène, Section~\ref{subsec:dataset})
qui domine le coût, pas la confidentialité différentielle ajoutée par-dessus.

La précision brute ne permet pas de départager les deux approches (82,0~\% contre
81,95~\%, dans le bruit statistique). L'écart se révèle sur les métriques équilibrées :
le centralisé conserve un pouvoir discriminant réel entre les deux classes
(balanced-accuracy $57{,}84\,\%$, nettement au-dessus du hasard à $50\,\%$), tandis que
le modèle fédéré global reste proche, voire légèrement en-dessous du hasard
($\approx 47{,}9\,\%$), signe qu'il tend à se replier sur la classe majoritaire plutôt
que de véritablement discriminer le stress. C'est ce chiffre -- environ 10 points de
balanced-accuracy -- qui constitue le coût mesuré de la confidentialité et de la
décentralisation sur ce jeu de données (voir Section~\ref{sec:discussion}).
...
\subsection{Pourquoi le centralisé conserve-t-il un avantage discriminant, et pourquoi
ce coût est-il acceptable ?}
\label{subsec:why_cent_wins}

Sur le test-set groupé, le centralisé conserve un avantage net et stable en
balanced-accuracy (57,84~\% contre $\approx$ 47,9~\% pour le FL,
Section~\ref{subsec:global_comparison}). Plutôt que de laisser cet écart comme deux
mécanismes hypothétiques non départagés, la décomposition du
Tableau~\ref{tab:dp_decomposition} tranche directement la question :

\begin{itemize}
  \item \textbf{Ce n'est pas (ou très peu) le bruit DP-SGD.} FL sans DP
  (48,68~\% de balanced-accuracy) et FL + DP (47,83~\%) sont à moins d'un point l'un
  de l'autre, un écart plus petit que leurs écarts-types respectifs. Le clipping de
  gradient et le bruit gaussien ajoutent donc un coût marginal, pas le coût principal.
  \item \textbf{C'est l'agrégation fédérée elle-même.} L'écart avec le centralisé
  (environ 9 points) est déjà presque entièrement présent en FedProx pur, sans aucun
  DP-SGD. Avec 15 clients dont la composition de classes varie énormément
  (Section~\ref{subsec:dataset}), moyenner des mises à jour locales aussi hétérogènes
  produit un modèle global qui se replie vers un compromis proche de la prédiction
  majoritaire plutôt que vers une frontière de décision fine -- ce que confirme la
  balanced-accuracy sous les 50~\% pour le modèle fédéré global, avec ou sans DP.
\end{itemize}

Ce résultat déplace le message principal : le coût mesuré n'est pas tant celui de la
\emph{confidentialité} que celui de la \emph{décentralisation elle-même} sur un jeu de
données aussi hétérogène -- la confidentialité différentielle s'ajoute ensuite
quasiment gratuitement.

\paragraph{Calibration du seuil de décision vs absence de signal.} Pour distinguer un
problème de calibration du seuil de classification d'une véritable absence de pouvoir
discriminant, nous balayons le seuil de $10^{-3}$ à $0{,}99$ sur les probabilités déjà
produites par les checkpoints existants (aucun réentraînement). Pour le centralisé, le
seuil optimal se situe dans la plage utile ($0{,}55$--$0{,}80$ selon la graine), avec un
gain modeste ($57{,}84\,\%~\to~58{,}31\,\%$) : un signal discriminant réel existe,
seulement légèrement mal calibré. Pour le FL+DP, en revanche, le seuil optimal
s'effondre à $0{,}001$ pour les 25 checkpoints sans exception -- soit prédire ``stress''
quasi systématiquement -- pour un gain minime ($47{,}86\,\%~\to~49{,}18\,\%$), toujours
au niveau du plancher théorique de la prédiction constante ($50\,\%$), sans jamais
l'atteindre. L'AUC-ROC, indépendante de tout choix de seuil, confirme et précise ce
diagnostic : $0{,}4470 \pm 0{,}0191$ pour le FL+DP contre $0{,}5379 \pm 0{,}0137$ pour le
centralisé. Une inspection par client révèle que cette AUC sous $0{,}5$ n'est pas un
effet diffus sur l'ensemble de la fédération : 99,9~\% des fenêtres où le modèle est
confiant à tort proviennent d'un seul client, dont la relation locale entre signaux
physiologiques et stress diverge apparemment du pattern appris sur le reste de la
population -- un cas concret de non-stationnarité Non-IID entre clients plutôt qu'une
inversion systémique de l'agrégation. Une AUC comparable sur l'ablation FL sans DP-SGD
($0{,}4662 \pm 0{,}0256$) confirme que cet effet est attribuable à la fédération
elle-même et non au bruit de confidentialité. Ce n'est donc pas un problème de
calibration mais une absence structurelle de pouvoir discriminant, concentrée sur les
clients dont la distribution s'écarte le plus du reste de la population, cohérente avec
la décomposition ci-dessus attribuant le coût à l'agrégation plutôt qu'à la
confidentialité.
...
Ce coût d'environ 10 points de balanced-accuracy n'invalide pas l'intérêt du FL dans ce
contexte : il en \textbf{quantifie honnêtement le prix}. Le centralisé n'est pas une
alternative réellement disponible pour ce cas d'usage -- agréger les signaux
physiologiques bruts de 15 infirmières sur un serveur unique constituerait un
traitement de données de santé difficilement compatible avec le principe de
minimisation des données du RGPD (Art.~5)~\cite{gdpr2016}, tel que rappelé en
introduction. Le chiffre mesuré ici sert donc de référence pour statuer si ce coût est
acceptable au regard du gain de conformité et de sécurité obtenu, et pour orienter les
travaux futurs (features plus informatives, personnalisation renforcée, réduction
ciblée du bruit sur les couches sensibles) visant à le réduire.
...
\begin{table}[h!]
  \centering
  \caption{FedPer vs FedProx+DP, test-set groupé (méthodologie d'évaluation propre à
  chaque architecture, voir texte) -- moyenne sur 5$\sigma\times$5 graines (n=25 chacun).}
  \label{tab:fedper_comparison}
  \begin{tabular}{lcc}
    \toprule
    Métrique & FedProx + DP & FedPer \\
    \midrule
    Accuracy               & $82{,}02 \pm 0{,}62\,\%$ & $\mathbf{83{,}21 \pm 1{,}65\,\%}$ \\
    Balanced-accuracy       & $47{,}86 \pm 0{,}35\,\%$ & $\mathbf{65{,}36 \pm 2{,}14\,\%}$ \\
    F1 macro                & $45{,}35 \pm 0{,}28\,\%$ & $\mathbf{65{,}34 \pm 2{,}43\,\%}$ \\
    AUC-ROC                  & $0{,}4470 \pm 0{,}0191$  & $\mathbf{0{,}7352 \pm 0{,}0366}$ \\
    Communication (FIT, moyenne) & $5{,}320$~Ko          & $5{,}188$~Ko ($-2{,}5\,\%$) \\
    \bottomrule
  \end{tabular}
\end{table}

La différence est statistiquement écrasante sur les 25 valeurs de chaque condition
(Welch : $t=40{,}36$, $p=1{,}7\times10^{-24}$ sur la balanced-accuracy ; Mann-Whitney
$U=625$ -- soit une séparation complète, chaque valeur FedPer dépasse chaque valeur
FedProx+DP sur les 625 paires possibles). La stabilité de la balanced-accuracy FedPer à
travers les 5 valeurs de $\sigma$ (64,8~\% à 65,9~\%, écart maximal $\approx 1{,}1$ point)
a été vérifiée spécifiquement pour cette architecture -- pas supposée par transfert de la
justification déjà établie pour FedProx -- avant de regrouper les 25 valeurs comme
réplicats d'une même condition. Le détail complet (résultats par $\sigma$ et par graine,
tests statistiques) est conservé dans \texttt{resultsfeat/fedper\_results\_summary.md},
séparément de ce document.

\paragraph{Interprétation -- une confirmation directe, pas un résultat isolé.} Ce résultat
complète exactement la décomposition FL-vs-DP ci-dessus : celle-ci avait attribué le
coût de balanced-accuracy à l'\emph{agrégation}, pas au bruit de confidentialité. FedPer
teste directement cette hypothèse en supprimant l'agrégation de la seule couche
responsable de la décision finale -- et l'essentiel du coût disparaît. L'ampleur de
l'écart (séparation complète des 25 valeurs -- vérifié également en écartant l'hypothèse
d'un mélange accidentel de checkpoints entre les deux conditions, et en confirmant que le
pire run FedPer dépasse encore le meilleur run FedProx+DP de 10,2 points, une marge large
et non fragile) ne doit pas surprendre, mais \textbf{ne doit pas non plus être imputée à
un facteur unique} : FedPer et FedProx diffèrent ici simultanément sur \emph{deux axes
non démêlés} par ce protocole -- (1)~la \textbf{durée} de personnalisation (30 rounds
continus pendant l'entraînement, contre 8 époques de fine-tuning post-hoc pour FedProx,
Tableau~\ref{tab:ablation_b}) et (2)~la \textbf{granularité} (seule la tête de
classification pour FedPer, contre le réseau entier pour le fine-tuning post-hoc de
FedProx). Ce travail ne permet pas de répondre à la question ``l'écart se refermerait-il
si FedProx était fine-tuné pendant 30 rounds complets au lieu de 8 époques ?'' -- les deux
facteurs n'ont jamais été isolés l'un de l'autre, faute de temps pour un protocole
supplémentaire dédié. Fait notable, l'accuracy brute progresse elle aussi légèrement
($+1{,}19$ point, $p=1{,}97\times10^{-3}$) : FedPer n'échange donc pas de la précision
brute contre du rappel équilibré, les deux progressent ensemble ici.
...
($\sigma \in [0{,}6;\, 1{,}8]$ sur 5 graines chacun) et deux ablations ciblées, montre
qu'il est possible de construire un système FL opérationnel qui : (1)~fournit une
garantie formelle $(\varepsilon, \delta)$-DP~\cite{dwork2014foundations} atteignant
$\varepsilon = 0{,}54 \pm 0{,}04$ à $\sigma = 1{,}8$ (sous $\varepsilon = 1$, un seuil
couramment retenu dans la littérature DP pour des applications sensibles) ;
(2)~quantifie honnêtement, sur un test-set groupé
méthodologiquement comparable, le coût de cette confidentialité et de cette
décentralisation par rapport à une baseline centralisée non déployable en pratique
sur ce type de données : environ 10 points de balanced-accuracy
(57,84~\% contre 47,9~\%), pour une précision brute statistiquement équivalente
(82,0~\% contre 81,95~\%) ; (2bis)~décompose ce coût grâce à une ablation FedProx
sans DP-SGD (Tableau~\ref{tab:dp_decomposition}) et montre qu'il est attribuable
presque entièrement à la décentralisation elle-même (FL sans DP : $48{,}68\,\%$ de
balanced-accuracy, à moins d'un point de FL+DP), le bruit de confidentialité
n'ajoutant qu'un coût marginal ; (3)~produit un modèle déployable sur microcontrôleur en
5,32~Ko (FP32), quantifiable dynamiquement en INT8 avec une dégradation de précision
quasi nulle, expliquée par le clipping de gradient DP-SGD qui resserre fortement la
plage dynamique des poids ; et (4)~présente un compromis confidentialité/utilité
empiriquement plat sur toute la plage testée $\varepsilon \in [0{,}54;\, 5{,}29]$ : une
garantie de confidentialité forte n'entraîne pas de coût supplémentaire mesurable par
rapport à une garantie faible ; et (5)~confirme directement la décomposition (2bis) en
testant une architecture qui supprime l'agrégation de la couche de classification
(FedPer, Section~\ref{subsec:fedper}) : la balanced-accuracy remonte à
$65{,}36 \pm 2{,}14\,\%$ (contre $47{,}86\,\%$ pour FedProx+DP standard, écart
statistiquement écrasant sur 25 valeurs), sans sacrifier l'accuracy brute (légèrement
supérieure, $+1{,}19$ point) ni le coût de communication (légèrement inférieur,
$-2{,}5\,\%$) -- au prix d'une contrainte de déploiement propre à cette architecture
(pas de modèle global unique, chaque client garde sa propre tête de classification) ;
et (6)~pousse cette logique plus loin hors contrainte TinyML (Section~\ref{subsec:fedrep_scaffold})
en combinant tête multi-couches, DP restreint à l'extracteur et SCAFFOLD : sur les
clients au test-set réellement mixte (la mesure la plus honnête cliniquement), la
hiérarchie FedProx+DP ($42{,}7\,\%$) $<$ FedPer ($48{,}4\,\%$) $<$ FedRep+SCAFFOLD
($59{,}9\,\%$, aussi nettement plus stable) est statistiquement nette -- au prix d'un
coût de calcul $\approx 5\times$ supérieur, non négligeable même hors contrainte de
taille de modèle.
...
\begin{itemize}
  ...
  \item \textbf{FedPer vs FedProx : deux facteurs confondus, non démêlés.} L'écart de
  balanced-accuracy en faveur de FedPer (Section~\ref{subsec:fedper}) reflète à la fois
  une différence de \emph{durée} de personnalisation (30 rounds continus contre 8
  époques post-hoc) et de \emph{granularité} (tête seule contre réseau entier
  fine-tuné) -- deux axes qui varient simultanément dans ce protocole. Isoler leur
  contribution respective (par exemple en fine-tunant FedProx pendant 30 rounds
  complets, ou en n'ajustant que sa dernière couche pendant 8 époques) nécessiterait un
  protocole dédié, non réalisé ici faute de temps.
  \item \textbf{Domination des métriques groupées par les clients mono-classe.}
  Découvert lors de l'évaluation de FedRep+SCAFFOLD (Section~\ref{subsec:fedrep_scaffold})
  mais s'appliquant rétroactivement à \emph{toutes} les métriques pooled sur 15 clients de
  ce document (Tableaux~\ref{tab:main_results}, \ref{tab:dp_decomposition},
  \ref{tab:fedper_comparison}) : 11 des 15 clients ont un test-set mono-classe, où une
  prédiction constante obtient mécaniquement $100\,\%$ quelle que soit la qualité du
  modèle. Ces clients dominent numériquement la balanced-accuracy groupée, diluant le
  signal des 4 clients au test-set réellement mixte (4, 5, 8, 12) -- où la performance
  réelle est sensiblement plus faible que ne le suggèrent les chiffres groupés sur 15
  clients. Les écarts \emph{relatifs} entre conditions (FL vs centralisé, FedProx vs
  FedPer vs FedRep+SCAFFOLD) restent probablement valables, chaque condition étant
  affectée de façon comparable par cet effet -- vérifié explicitement pour la comparaison
  FedPer/FedRep+SCAFFOLD (Section~\ref{subsec:fedrep_scaffold}), pas pour les autres
  tableaux. Une réévaluation systématique restreinte aux clients informatifs serait
  nécessaire pour des chiffres absolus pleinement fiables.
\end{itemize}
